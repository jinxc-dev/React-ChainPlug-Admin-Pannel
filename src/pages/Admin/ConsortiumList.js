...
class CertificateDocumentList extends Component {
  ...
  columns = [
    {
      ...
      filters: [
        {
          text: formatMessage({ id: 'admin.iot.label' }),
          value: 'IoT',
        },
        {
          text: formatMessage({ id: 'admin.coldchain.label' }),
          value: 'ColdChain',
        },
        {
            text: formatMessage({ id: 'admin.certificate.label' }),
            value: 'Certificate',
        }
      ],
      render(val) {
        return (
          <div style={{ ...statusStyles[val], ...{ borderRadius: '3px', textAlign: 'center' } }}>
            {formatMessage({ id: `admin.${val.toLowerCase()}.label` })}
          </div>
        );
      },
    },
  ...
  fechData() {
    const { consortiums } = this.props;
    const { status } = this.state;
    const newconsortiums = consortiums
      .map((comp, index) => {
        return {
          ...comp,
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper title={<FormattedMessage id="admin.list.consortium" />}>
        <Card bordered={false}>
